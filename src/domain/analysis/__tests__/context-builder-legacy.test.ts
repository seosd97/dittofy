...
import { buildContext, estimateTokens } from "@domain/analysis/context-builder.js"
...
describe("buildContext", () => {
	...
	it("includes all selected files within budget", () => {
		...
		const result = buildContext(chunks, emptyTree, {
			filePaths: ["src/Button.tsx", "tailwind.config.ts"],
		})
	...
	it("respects token budget and does not exceed it", () => {
		...
		const result = buildContext(chunks, emptyTree, {
			filePaths: ["src/A.tsx", "src/B.tsx", "src/C.tsx"],
			tokenBudget: 100,
		})

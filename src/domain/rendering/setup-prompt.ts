...
import { buildFileStructureGuide } from "./resolve-structure.js"
...
export function renderSetupPrompt(ctx: PromptTemplateContext): string {
	...
	const tokenCategories = analysis.designTokens
		? [
				(analysis.designTokens.colorGroups?.length ?? 0) > 0 && "Colors",
...
function buildExistingProjectInstructions(
	env: import("./resolve-environment.js").EnvironmentProfile,
): string {

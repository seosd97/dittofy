...
function createMinimalAnalysis(overrides: Partial<AnalysisResult> = {}): AnalysisResult {
	return {
		...
		designTokens: { colors: [], spacing: [], borderRadius: [], shadows: [], breakpoints: [], zIndex: [] },
		typography: { fontFamilies: { value: ["Inter"], confidence: "high" }, scale: [], lineHeights: [], fontWeights: [] },
		componentCatalog: { components: [], patterns: [] },
		layoutSystem: { approach: { value: "flexbox", confidence: "high" }, containers: [], grids: [], navigation: [] },
		pageStructures: { pages: [] },
		responsiveStrategy: { approach: { value: "mobile-first", confidence: "high" }, breakpoints: [], patterns: [] },
...
describe("planDocuments", () => {
	...
	it("includes page structures when pages exist", () => {
		const plan = planDocuments(
			createMinimalAnalysis({
				pageStructures: {
					pages: [
						{ name: "Home", route: "/", layout: "default", sections: [], components: [], confidence: "high" },
					],
				},
			}),
		)
	...
	it("includes interactions doc when animations exist", () => {
		const plan = planDocuments(
			createMinimalAnalysis({
				interactionPatterns: {
					animations: [{ name: "fadeIn", type: "entrance", description: "Fade in", confidence: "high" }],
					transitions: [],
					gestures: [],
				},
			}),
	...
	it("overview is always included even if all analyzers failed", () => {
		const plan = planDocuments(
			createMinimalAnalysis({
				failedAnalyzers: ["designTokens", "typography", "componentCatalog", "layoutSystem", "pageStructures", "responsiveStrategy", "interactionPatterns"],
			}),
		)

...
function createAnalysis(overrides: Partial<AnalysisResult> = {}): AnalysisResult {
	return {
		...
		typography: {
			...
			scale: [{ name: "h1", fontSize: "2.25rem", lineHeight: "2.5rem", fontWeight: "700", usage: "Page titles", confidence: "high" }],
		...
		pageStructures: {
			pages: [
				{ name: "Home", route: "/", layout: "default", sections: ["hero", "features"], components: ["Button", "Card"], confidence: "high" },
			],
		},
		responsiveStrategy: {
			approach: { value: "mobile-first", confidence: "high" },
			breakpoints: [{ name: "sm", value: "640px", confidence: "high" }],
			patterns: [{ name: "stack-to-grid", description: "Stack on mobile, grid on desktop", breakpoint: "sm", confidence: "high" }],
		},
		interactionPatterns: {
			animations: [{ name: "fadeIn", type: "entrance", description: "Fade in on mount", confidence: "high" }],
			transitions: [{ property: "opacity", duration: "200ms", easing: "ease-in-out", confidence: "high" }],
...
const docsWithDesign: DocumentSet = {
	documents: [
		{ filename: "01-design-tokens.md", title: "Design Tokens", content: "Token doc content", category: "core" },
		{ filename: "02-typography.md", title: "Typography", content: "Typography doc content", category: "core" },
		{ filename: "03-component-catalog.md", title: "Components", content: "Component doc content", category: "core" },
	],
	outputDir: "",
}
...
describe("injectContext", () => {
	describe("setup context", () => {
		it("includes tech stack and design essence", () => {
			const step: StepPlanEntry = { stepNumber: 1, stepType: "setup", title: "Setup", scope: "", dependencies: [] }
	...
	describe("design-system context", () => {
		it("includes tokens and typography", () => {
			const step: StepPlanEntry = { stepNumber: 2, stepType: "design-system", title: "DS", scope: "", dependencies: [1] }
		...
		it("handles null tokens gracefully", () => {
			const step: StepPlanEntry = { stepNumber: 2, stepType: "design-system", title: "DS", scope: "", dependencies: [1] }
			const ctx = injectContext(step, createAnalysis({ designTokens: null, typography: null }), emptyDocSet)
	...
	describe("components context", () => {
		it("includes only specified components via componentNames", () => {
			const step: StepPlanEntry = {
				stepNumber: 3, stepType: "components", title: "Components",
				scope: "Button", dependencies: [1, 2], componentNames: ["Button"],
			}
			const ctx = injectContext(step, createAnalysis(), emptyDocSet)
		...
		it("returns fallback when no matching components", () => {
			const step: StepPlanEntry = {
				stepNumber: 3, stepType: "components", title: "Components",
				scope: "", dependencies: [1, 2], componentNames: ["NonExistent"],
			}
			const ctx = injectContext(step, createAnalysis(), emptyDocSet)
	...
	describe("pages context", () => {
		it("includes page structures and layout system", () => {
			const step: StepPlanEntry = { stepNumber: 4, stepType: "pages", title: "Pages", scope: "", dependencies: [1, 2, 3] }
			const ctx = injectContext(step, createAnalysis(), emptyDocSet)
	...
	describe("responsive context", () => {
		it("includes breakpoints and patterns", () => {
			const step: StepPlanEntry = { stepNumber: 5, stepType: "responsive", title: "Responsive", scope: "", dependencies: [1, 2] }
			const ctx = injectContext(step, createAnalysis(), emptyDocSet)
	...
	describe("interactions context", () => {
		it("includes animations, transitions, and gestures", () => {
			const step: StepPlanEntry = { stepNumber: 6, stepType: "interactions", title: "Interactions", scope: "", dependencies: [1, 2] }
			const ctx = injectContext(step, createAnalysis(), emptyDocSet)
	...
	describe("generic context (unknown stepType)", () => {
		it("falls back to essence + all documents", () => {
			const step: StepPlanEntry = { stepNumber: 7, stepType: "setup", title: "Custom", scope: "", dependencies: [] }

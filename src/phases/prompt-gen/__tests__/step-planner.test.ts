...
import type { AnalysisResult, ComponentInfo } from "../../../types/analysis.js"
...
describe("planSteps", () => {
	...
	it("adds page step with correct dependencies", () => {
		const plan = planSteps(
			createMinimalAnalysis({
				componentCatalog: { components: [makeComponent("Button"), makeComponent("Card")], patterns: [] },
				pageStructures: {
					pages: [
		...
		const plan = planSteps(
			createMinimalAnalysis({
				responsiveStrategy: {
					...
					patterns: [{ name: "stack", description: "Stack on mobile", breakpoint: "sm", confidence: "high" }],
				},
			}),
		)
	...
	it("adds interactions step when animations exist", () => {
		const plan = planSteps(
			createMinimalAnalysis({
				interactionPatterns: {
					animations: [{ name: "fadeIn", type: "entrance", description: "Fade", confidence: "high" }],
					transitions: [],
					gestures: [],
				},
			}),
	...
	it("enforces max 12 steps", () => {
		...
		const plan = planSteps(
			createMinimalAnalysis({
				componentCatalog: { components, patterns: [] },
				pageStructures: {
					pages: [{ name: "Home", route: "/", layout: "default", sections: [], components: [], confidence: "high" }],
				},
				responsiveStrategy: {
					approach: { value: "mobile-first", confidence: "high" },
					breakpoints: [{ name: "sm", value: "640px", confidence: "high" }],
